...
<div class="space-y-6">
    ...
    @if (session('success'))
        <div class="rounded-xl border bg-emerald-50 border-emerald-200 text-emerald-700 dark:bg-emerald-500/15 dark:border-emerald-500/30 dark:text-emerald-200">
    ...
    <div class="rounded-2xl border bg-white shadow-card border-slate-200 dark:bg-[#0f172a] dark:border-[rgba(148,163,184,.12)]">
        <div class="p-6 md:p-7 space-y-6">
            <div class="flex items-center justify-between">
                ...
                <span class="text-xs px-2.5 py-1 rounded-full bg-slate-100 border border-slate-200 dark:bg-white/5 dark:border-[rgba(148,163,184,.12)]">Ref: ADJUST</span>
            </div>

            {{-- Pemilihan Cabang (jika user tidak punya cabang default) --}}
            @if (empty($active_branch))
                <div class="space-y-2">
                    <label class="block text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">Cabang</label>
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400">Sistem akan memakai/membuat lokasi <b>AVAILABLE</b> untuk cabang terpilih.</p>
                </div>
            @else
                <div class="flex items-center gap-3 text-sm">
                    <span class="text-slate-600 dark:text-slate-400">Cabang:</span>
                    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-lg border bg-slate-100 border-slate-200 dark:bg-white/5 dark:border-[rgba(148,163,184,.12)]">
            ...
            <form id="form-adjust" method="post" action="{{ route('stock.adjust.store') }}" class="space-y-6">
                ...
                {{-- Produk --}}
                <div class="space-y-2">
                    <label class="block text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">Produk</label>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">Jumlah Seharusnya (Qty Baru)</label>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-400">Masukkan jumlah stok fisik yang ada saat ini. Sistem akan menghitung selisihnya secara otomatis.</p>
                    </div>

                    {{-- Alasan Penyesuaian --}}
                    <div class="space-y-2">
                        <label class="block text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">Alasan Penyesuaian</label>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="submit" id="submitAdjust"
                            class="inline-flex items-center gap-2 h-10 px-4 rounded-xl border text-white bg-brand hover:bg-brand/90 border-transparent dark:bg-brandDark dark:hover:bg-brandDark/90">
                        Simpan Penyesuaian
                    </button>

                    <details class="group">
                        <summary class="flex items-center gap-2 cursor-pointer select-none px-3 py-2 rounded-xl border hover:bg-slate-100 border-slate-200 dark:hover:bg-white/5 dark:border-[rgba(148,163,184,.12)]">
                        ...
                        <div class="mt-2 text-sm text-slate-600 dark:text-slate-400 px-1">
